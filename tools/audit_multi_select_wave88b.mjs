...
assert(/^(wave88[bcd]|wave89[a-z]+)$/i.test(String(healthz.wave || '')), `healthz.json: expected wave88b+ or a later wave89* build, got ${healthz.wave}`);
assert(/^(wave88[bcd]|wave89[a-z]+)$/i.test(String(healthz.build_id || '')), `healthz.json: expected build_id wave88b+ or a later wave89* build, got ${healthz.build_id}`);

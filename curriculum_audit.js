...
for (const file of gradeFiles) {
  ...
  subj.forEach(s => {
    ...
    const allowThin = s.id === 'eng' && ['grade2_v2.html','grade3_v2.html','grade4_v2.html'].includes(file);
    if (topicCount < 3 && !s.locked && !allowThin) thinBlocks.push({ file, subject: s.nm, topicCount });
    s.tops.forEach(t => {
      topics++;

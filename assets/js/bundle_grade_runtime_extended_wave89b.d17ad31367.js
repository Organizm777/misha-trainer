...
(function(){
    ...
    var sections = [
      ...
      },
      {
        ...
        items: [
          ...
          { id:'qr-transfer', icon:'📱', label:'QR-перенос', note:'Показать QR-пакет резервной копии для другого устройства' },
          { id:'telegram-sync', icon:'✈️', label:'Telegram sync', note:'Скопировать backup и открыть Telegram для переноса' },
          { id:'data-health', icon:'🩺', label:'Проверка данных', note:'Integrity, квота, auto-backup и Service Worker' },
  ...
  function runMenuAction(id){
    ...
    if (id === 'qr-transfer' || id === 'telegram-sync' || id === 'data-health') {
      closeThen(function(){
        loadServices({ ui:{ scope:'runtime', kind:'services', action:id, title:'Подгружаю инструменты данных', label:'Готовлю QR, Telegram и проверку локального хранилища…' } }).then(function(){
          var tools = root.wave93eDataTools;
          if (tools && typeof tools.open === 'function') tools.open(id === 'qr-transfer' ? 'qr' : (id === 'telegram-sync' ? 'telegram' : 'status'));
          else toast('Инструменты данных пока загружаются.');
        });
      });
      return true;
    }

...
(function(){
  ...
  var EXAM_AUTOSAVE_KEY = 'trainer_exam_autosave_wave92b';
  ...
  function cloneQuestion(row, meta){
    var out = {
      ...
      solution_steps: Array.isArray(row.solution_steps) ? row.solution_steps.slice() : (Array.isArray(row.solutionSteps) ? row.solutionSteps.slice() : []),
      code: row.code || '',
      isMath: !!row.isMath,
  ...
  function examSubjectLabel(subjectId, packLabel){
    var map = {
      ...
      history: 'История',
      his: 'История',
      base_math: 'Математика',
      profile_math: 'Математика'
    };
  ...
  /* wave91i: informatics/history structured exam packs */
  function makeWave91iStructuredPack(idx, cfg){
    return {
      id: cfg.prefix + idx,
      exam: cfg.exam,
      label: cfg.label + ' · Вариант ' + idx,
      subjectId: cfg.subjectId,
      fallbackSubjects: [cfg.subjectId],
      accent: cfg.accent,
      grades: cfg.grades,
      minG: cfg.minG,
      maxG: cfg.maxG,
  ...
  function attachWave91iExamPacks(){
    if (EXAM_PACKS.oge_informatics) EXAM_PACKS.oge_informatics.hidden = true;
    var ogeInfo = { prefix:'oge_informatics_var', exam:'ОГЭ', label:'ОГЭ · Информатика', subjectId:'informatics', accent:'#0f172a', grades:'9 класс', minG:8, maxG:9, maxQ:15, timeLimit:150*60, scoreKind:'grade', scoreModel:'oge_informatics_2026', order:220, summary:'15 заданий · системы счисления, логика, таблицы, Python', bands:[{min:0,label:'2',note:'ниже порога'},{min:6,label:'3',note:'базовый уровень'},{min:10,label:'4',note:'хороший уровень'},{min:13,label:'5',note:'сильный результат'}] };
    var egeInfo = { prefix:'ege_informatics_var', exam:'ЕГЭ', label:'ЕГЭ · Информатика', subjectId:'informatics', accent:'#1e293b', grades:'10–11 класс', minG:10, maxG:11, maxQ:15, timeLimit:235*60, scoreKind:'ege100', scoreModel:'ege_informatics_part1_2026', order:320, summary:'15 заданий · кодирование, логика, алгоритмы, таблицы, сети', bands:[{min:0,label:'ниже порога',note:'нужно добрать базу'},{min:6,label:'порог',note:'минимальный ориентир'},{min:9,label:'рабочий',note:'уверенная база'},{min:12,label:'сильный',note:'сильный результат'},{min:14,label:'высокий',note:'высокий результат'}] };
    var ogeHist = { prefix:'oge_history_var', exam:'ОГЭ', label:'ОГЭ · История', subjectId:'history', accent:'#92400e', grades:'9 класс', minG:8, maxG:9, maxQ:24, timeLimit:180*60, scoreKind:'grade', scoreModel:'oge_history_2026', order:260, summary:'24 задания · даты, персоналии, источники, причинно-следственные связи', bands:[{min:0,label:'2',note:'ниже порога'},{min:10,label:'3',note:'базовый уровень'},{min:16,label:'4',note:'хороший уровень'},{min:21,label:'5',note:'сильный результат'}] };
    var egeHist = { prefix:'ege_history_var', exam:'ЕГЭ', label:'ЕГЭ · История', subjectId:'history', accent:'#b45309', grades:'10–11 класс', minG:10, maxG:11, maxQ:20, timeLimit:210*60, scoreKind:'ege100', scoreModel:'ege_history_part1_2026', order:360, summary:'20 заданий · даты, процессы, культура, аргументация', bands:[{min:0,label:'ниже порога',note:'нужно добрать базу'},{min:7,label:'порог',note:'минимальный ориентир'},{min:11,label:'рабочий',note:'уверенная база'},{min:15,label:'сильный',note:'сильный результат'},{min:18,label:'высокий',note:'высокий результат'}] };
    structuredVariantNosHint('oge_informatics_2026_full', 30).forEach(function(i){ EXAM_PACKS['oge_informatics_var' + i] = makeWave91iStructuredPack(i, ogeInfo); });
    structuredVariantNosHint('ege_informatics_2026_part1', 30).forEach(function(i){ EXAM_PACKS['ege_informatics_var' + i] = makeWave91iStructuredPack(i, egeInfo); });
    structuredVariantNosHint('oge_history_2026_full', 20).forEach(function(i){ EXAM_PACKS['oge_history_var' + i] = makeWave91iStructuredPack(i, ogeHist); });
    structuredVariantNosHint('ege_history_2026_part1', 20).forEach(function(i){ EXAM_PACKS['ege_history_var' + i] = makeWave91iStructuredPack(i, egeHist); });
  }
  attachWave91iExamPacks();
  ...
  var STRUCTURED_FAMILY_SPECS = {
    ...
    oge_informatics_2026_full: { pattern: /^oge_informatics_var(\d+)$/, exam:'ОГЭ', subject:'informatics', year:2026, mode:'full' },
    ege_informatics_2026_part1: { pattern: /^ege_informatics_var(\d+)$/, exam:'ЕГЭ', subject:'informatics', year:2026, mode:'part1' },
    oge_history_2026_full: { pattern: /^oge_history_var(\d+)$/, exam:'ОГЭ', subject:'history', year:2026, mode:'full' },
    ege_history_2026_part1: { pattern: /^ege_history_var(\d+)$/, exam:'ЕГЭ', subject:'history', year:2026, mode:'part1' }
  };
  ...
  function renderStructuredBankSkeleton(packId){
    try {
      ensureStyle();
      var pack = EXAM_PACKS[packId] || {};
      var host = document.getElementById('wave30-pack-host') || document.getElementById('subj-grid') || document.body;
      if (!host) return;
      var sk = document.getElementById('wave92b-exam-skeleton');
      if (!sk) { sk = document.createElement('div'); sk.id = 'wave92b-exam-skeleton'; sk.className = 'wave92b-exam-skeleton'; host.parentNode ? host.parentNode.insertBefore(sk, host.nextSibling) : host.appendChild(sk); }
      sk.innerHTML = '<div class="wave92b-sk-title">Загружаю ' + esc(pack.label || 'экзаменный банк') + '</div><div class="wave92b-sk-line"></div><div class="wave92b-sk-line small"></div><div class="wave92b-sk-grid"><i></i><i></i><i></i><i></i></div>';
    } catch(_) {}
  }
  function clearStructuredBankSkeleton(){ try { var sk = document.getElementById('wave92b-exam-skeleton'); if (sk) sk.remove(); } catch(_) {} }
  ...
  function notifyStructuredBankLoading(packId){
    try {
      ...
      renderStructuredBankSkeleton(packId);
  ...
  function structuredRowFromQuestion(pack, built, question, variantNo, spec){
    ...
    return {
      ...
      solution_steps: Array.isArray(question.solution_steps) ? question.solution_steps.slice() : (Array.isArray(question.solutionSteps) ? question.solutionSteps.slice() : []),
      topic_tag: structuredTopicTag(question.section || question.topic),
  ...
  function buildStructuredKim(familyId, variantNo){
    ...
    var questions = pickedRows.map(function(row){
      var meta = {
        ...
        solution_steps: Array.isArray(row.solution_steps) ? row.solution_steps.slice() : [],
  ...
  function readExamAutosave(){
    var data = safeJSON(EXAM_AUTOSAVE_KEY, null);
    return data && typeof data === 'object' ? data : null;
  }
  function clearExamAutosave(packId){
    try {
      var data = readExamAutosave();
      if (!packId || (data && data.packId === packId)) localStorage.removeItem(EXAM_AUTOSAVE_KEY);
    } catch(_) {}
  }
  function saveExamAutosave(reason){
    try {
      ...
      saveJSON(EXAM_AUTOSAVE_KEY, {
        ...
        answers: answerEntries().slice(),
        uiState: window.__wave30ExamUiState || null,
        remaining: typeof session.remaining === 'number' ? session.remaining : null,
        startedAt: session.startedAt || null,
        total: packQuestionCount(pack)
      });
    } catch(_) {}
  }
  function restoreExamAutosave(pack){
    try {
      var data = readExamAutosave();
      if (!pack || !data || data.packId !== pack.id) return false;
      if (Date.now() - toNum(data.ts) > 7 * 24 * 60 * 60 * 1000) { clearExamAutosave(pack.id); return false; }
      if (typeof answers !== 'undefined' && Array.isArray(answers) && Array.isArray(data.answers)) answers = data.answers.slice();
      if (typeof qIndex !== 'undefined') qIndex = Math.max(0, Math.min(toNum(data.qIndex), Math.max(0, pack.maxQ - 1)));
      if (data.uiState && typeof data.uiState === 'object') window.__wave30ExamUiState = data.uiState;
      if (window.__wave25DiagSession && typeof data.remaining === 'number') window.__wave25DiagSession.remaining = data.remaining;
      window.__wave92bExamRestored = true;
      return true;
  ...
  function ensureStyle(){
    ...
    style.textContent = '' +
      ...
      '.wave30-quiz-meta{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}.wave30-quiz-chip{display:inline-flex;align-items:center;gap:6px;padding:6px 9px;border-radius:999px;background:var(--card);border:1px solid var(--border);font-size:10px;font-weight:700;color:var(--muted)}.wave30-quiz-chip b{color:var(--text)}.wave92b-exam-skeleton{margin:12px 0;padding:14px;border:1px solid var(--border);border-radius:16px;background:var(--card);overflow:hidden}.wave92b-sk-title{font-size:12px;font-weight:900;margin-bottom:10px}.wave92b-sk-line,.wave92b-sk-grid i{display:block;height:12px;border-radius:999px;background:linear-gradient(90deg,rgba(148,163,184,.20),rgba(148,163,184,.40),rgba(148,163,184,.20));animation:wave92bsk 1.2s infinite}.wave92b-sk-line.small{width:60%;margin-top:8px}.wave92b-sk-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:12px}.wave92b-sk-grid i{height:34px}@keyframes wave92bsk{0%{opacity:.45}50%{opacity:1}100%{opacity:.45}}' +
      '.wave30-solutions{margin-top:12px}.wave30-solution{border:1px solid var(--border);border-radius:14px;background:var(--card);padding:10px;margin-top:8px}.wave30-solution summary{cursor:pointer;font-size:12px;font-weight:900}.wave30-solution-q{font-size:12px;line-height:1.45;margin-top:8px}.wave30-solution-meta{font-size:11px;color:var(--muted);margin-top:6px}.wave30-solution ol{margin:8px 0 0 18px;padding:0}.wave30-solution li{font-size:12px;line-height:1.45;margin:4px 0}' +
  ...
  function updateQuizMeta(){
    ...
    holder.innerHTML = '' +
      ...
      '<div class="wave30-quiz-toolbar">' +
        ...
        '<span class="wave30-quiz-counter">Отвечено ' + answered + '/' + total + (flagged ? (' · отмечено ' + flagged) : '') + (currentRemainingText() ? (' · осталось ' + esc(currentRemainingText())) : '') + (window.__wave92bExamRestored ? ' · восстановлено из autosave' : ' · autosave') + '</span>' +
      '</div>' +
  ...
  function solutionStepsForQuestion(question){
    if (!question) return [];
    if (Array.isArray(question.solution_steps) && question.solution_steps.length) return question.solution_steps.slice();
    if (question.structuredBankRow && Array.isArray(question.structuredBankRow.solution_steps)) return question.structuredBankRow.solution_steps.slice();
    var out = [];
    if (question.hint) out.push(question.hint);
    if (question.ex) out.push(question.ex);
    if (question.a) out.push('Правильный ответ: ' + question.a);
    return out.filter(Boolean);
  }
  function buildSolutionStepsReview(){
    try {
      if (typeof questions === 'undefined' || !Array.isArray(questions) || !questions.length) return '';
      var byQi = {};
      (answers || []).forEach(function(ans){ if (ans && typeof ans.qi !== 'undefined') byQi[String(ans.qi)] = ans; });
      var rows = [];
      questions.forEach(function(question, idx){
        var entry = byQi[String(idx)];
        var steps = solutionStepsForQuestion(question);
        if (!steps.length) return;
        if (!entry || !entry.correct) rows.push({ question:question, idx:idx, entry:entry, steps:steps, weak:true });
      });
      if (!rows.length) {
        questions.slice(0, 5).forEach(function(question, idx){
          var steps = solutionStepsForQuestion(question);
          if (steps.length) rows.push({ question:question, idx:idx, entry:byQi[String(idx)], steps:steps, weak:false });
        });
      }
      rows = rows.slice(0, 8);
      if (!rows.length) return '';
      return '<div class="wave30-solutions"><div class="wave30-sec-band"><b>Пошаговый разбор:</b> ' + (rows[0].weak ? 'сначала показаны задания с ошибками или без ответа.' : 'ошибок почти нет, показаны первые задания для самопроверки.') + '</div>' + rows.map(function(row){
        var q = row.question || {};
        var chosen = row.entry && row.entry.chosenText ? row.entry.chosenText : 'нет ответа';
        return '<details class="wave30-solution"><summary>Задание ' + esc(String(q.taskNo || (row.idx + 1))) + ' · ' + esc(q.section || q.topic || 'раздел') + '</summary>' +
          '<div class="wave30-solution-q">' + esc(q.q || '') + '</div>' +
          '<div class="wave30-solution-meta">Ответ ученика: ' + esc(chosen) + ' · верно: ' + esc(q.a || '') + '</div>' +
          '<ol>' + row.steps.map(function(step){ return '<li>' + esc(step) + '</li>'; }).join('') + '</ol></details>';
      }).join('') + '</div>';
    } catch(_err) { return ''; }
  }
  ...
  function patchStart(){
    ...
    window.startDiag = function(subjId){
      ...
      var wave92bRestored = restoreExamAutosave(packRuntime);
      closeExamReview(true);
      ...
      if (typeof window.renderQ === 'function') window.renderQ();
      updateQuizMeta();
      saveExamAutosave(wave92bRestored ? 'restored' : 'start');
      return out;
    };
  ...
  function auditSnapshot(){
    ...
    return {
      ...
      features: {
        ...
        hasWave92bAutosave: true,
        hasWave92bSkeleton: true,

...
/* wave92f: SM-2 in the main training stream */
(function(){
  'use strict';
  var root = window;
  if (!root || root.__wave92fSm2Main) return;
  root.__wave92fSm2Main = true;
  function grade(){ try { return String(root.GRADE_NUM || root.GRADE_NO || '10'); } catch(_) { return '10'; } }
  var KEY = 'trainer_sm2_wave92f_' + grade();
  function now(){ return Date.now(); }
  function getLS(){ try { var raw = localStorage.getItem(KEY); var data = raw ? JSON.parse(raw) : {}; return data && typeof data === 'object' ? data : {}; } catch(_) { return {}; } }
  function setLS(data){ try { localStorage.setItem(KEY, JSON.stringify(data)); } catch(_) {} try { if (root.trainerStore && trainerStore.set) trainerStore.set(KEY, JSON.stringify(data)); } catch(_) {} }
  function txt(v){ return String(v == null ? '' : v).replace(/\s+/g,' ').trim(); }
  function qkey(q){ return (txt(q && (q.tag || q.topic)) + '|' + txt(q && (q.question || q.q)) + '|' + txt(q && (q.answer || q.a))).toLowerCase().slice(0, 260); }
  function cloneQ(q){ return { question:txt(q.question || q.q), answer:txt(q.answer || q.a), options:Array.isArray(q.options) ? q.options.slice(0,4) : (Array.isArray(q.o) ? q.o.slice(0,4) : []), hint:txt(q.hint || q.h), ex:txt(q.ex || q.explanation), tag:txt(q.tag || q.topic), color:q.color || '#2563eb', bg:q.bg || '#dbeafe', code:q.code || null, isMath:!!q.isMath, _wave92fSm2Review:true }; }
  function update(q, ok){
    if (!q || q._wave92fNoSm2) return;
    var key = qkey(q); if (!key) return;
    var data = getLS(), row = data[key] || { reps:0, lapses:0, interval:0, ease:2.5, due:now(), createdAt:now() };
    var quality = ok ? 5 : 2;
    if (quality < 3) { row.reps = 0; row.lapses = (+row.lapses || 0) + 1; row.interval = 1; }
    else { row.reps = (+row.reps || 0) + 1; row.interval = row.reps === 1 ? 1 : (row.reps === 2 ? 3 : Math.max(4, Math.round((+row.interval || 3) * (+row.ease || 2.5)))); }
    row.ease = Math.max(1.3, (+row.ease || 2.5) + (0.1 - (5 - quality) * (0.08 + (5 - quality) * 0.02)));
    row.due = now() + row.interval * 86400000;
    row.updatedAt = now(); row.lastOk = !!ok; row.payload = cloneQ(q);
    data[key] = row; setLS(data);
  }
  function due(){
    var data = getLS(), t = now(), arr = Object.keys(data).map(function(k){ var r=data[k]; return r && r.payload && r.due <= t ? r : null; }).filter(Boolean);
    arr.sort(function(a,b){ return (a.due||0)-(b.due||0); });
    return arr[0] && arr[0].payload ? arr[0].payload : null;
  }
...
/* wave92f: parameterized question generator */
(function(){
  'use strict';
  var root = window;
  if (!root || root.__wave92fParametric) return;
  root.__wave92fParametric = true;
  function txt(v){ return String(v == null ? '' : v).replace(/\s+/g,' ').trim(); }
  function seed(){ var x = Date.now() + Math.floor(Math.random()*9973) + (+root.GRADE_NUM || 0)*101; return function(min,max){ x = (x * 1664525 + 1013904223) >>> 0; return min + (x % (max-min+1)); }; }
  function opts(ans, list){ var a=[String(ans)].concat(list.map(String)).filter(function(x,i,r){return x!==''&&r.indexOf(x)===i}); while(a.length<4)a.push(String(Number(ans||0)+a.length+1)); for(var i=a.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),t=a[i];a[i]=a[j];a[j]=t;} return a.slice(0,4); }
  function mk(q,a,o,h,tag,ex,isMath){ if (typeof root.mkQ === 'function') return root.mkQ(q,a,o,h,tag,'#2563eb','#dbeafe',null,!!isMath,ex); return {question:q,answer:String(a),options:opts(a,o),hint:h,tag:tag,color:'#2563eb',bg:'#dbeafe',ex:ex,isMath:!!isMath}; }
  var templates = [
    { id:'linear_equation', re:/alg|math|мат|алг|урав|eq/i, build:function(r,meta){ var a=r(2,9), x=r(2,12), b=r(1,20), c=a*x+b; return mk('Параметрическое задание: реши уравнение '+a+'x + '+b+' = '+c+'.', x, opts(x,[x-1,x+1,c-b,a+b]), 'Перенеси свободный член и раздели на коэффициент при x.', meta.topic || 'Уравнения', 'Вычитаем '+b+': '+a+'x = '+(c-b)+'. Делим на '+a+': x = '+x+'.', true); }},
    { id:'probability', re:/prob|вероят|стат|комбин/i, build:function(r,meta){ var red=r(2,8), blue=r(3,9), total=red+blue, ans=red+'/'+total; return mk('Параметрическое задание: в коробке '+red+' красных и '+blue+' синих карточек. Какова вероятность достать красную?', ans, [blue+'/'+total, red+'/'+blue, '1/'+total], 'Вероятность = благоприятные исходы / все исходы.', meta.topic || 'Вероятность', 'Всего карточек '+total+', благоприятных '+red+', значит P = '+ans+'.', true); }},
    { id:'physics_speed', re:/phy|физ|кинемат|скор/i, build:function(r,meta){ var v=r(2,12), t=r(3,9), s=v*t; return mk('Параметрическое задание: тело движется равномерно со скоростью '+v+' м/с в течение '+t+' с. Какой путь оно пройдёт?', s+' м', [(s+v)+' м',(s-t)+' м',v+t+' м'], 'Используй формулу s = v·t.', meta.topic || 'Кинематика', 's = '+v+'·'+t+' = '+s+' м.', true); }},
    { id:'chem_molar', re:/chem|хим|моль|атом/i, build:function(r,meta){ var n=r(2,6), m=18*n; return mk('Параметрическое задание: сколько граммов воды H₂O соответствует '+n+' моль вещества?', m+' г', [(m+18)+' г',(m-18)+' г',n+' г'], 'Молярная масса воды равна 18 г/моль.', meta.topic || 'Химия', 'm = n·M = '+n+'·18 = '+m+' г.', true); }},
    { id:'informatics_logic', re:/inf|информ|logic|логик|python/i, build:function(r,meta){ var a=!!r(0,1), b=!!r(0,1), ans=(a && !b) ? 'Истина' : 'Ложь'; return mk('Параметрическое задание: A = '+(a?'истина':'ложь')+', B = '+(b?'истина':'ложь')+'. Чему равно выражение A И НЕ B?', ans, ['Истина','Ложь','Нельзя определить','Ошибка записи'], 'Сначала вычисли НЕ B, затем логическое И.', meta.topic || 'Логика', 'НЕ B = '+(!b?'истина':'ложь')+', поэтому A И НЕ B = '+ans.toLowerCase()+'.', false); }}
  ];
  function meta(){ var s=null,t=null; try { s = typeof cS !== 'undefined' ? cS : root.cS; } catch(_) {} try { t = typeof cT !== 'undefined' ? cT : root.cT; } catch(_) {} return { subj:txt((s && (s.id+' '+s.nm)) || ''), topic:txt((t && (t.id+' '+t.nm)) || '') }; }
...
(function(){
  ...
  function inputModeFor(question){
    return ''; // wave92e: DISABLED — all questions stay multiple choice (A/B/C/D).
  }

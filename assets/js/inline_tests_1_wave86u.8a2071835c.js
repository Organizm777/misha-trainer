...
// Spatial (10)
SP('Сколько граней у обычного куба?','6',['4','8','12'],'У куба 6 квадратных граней',1);SP('В развёртке обычного куба сколько квадратных граней должно быть?','6',['4','5','8'],'Каждая грань куба — один квадрат',1);SP('Стрелка смотрит вверх. Её повернули на 90° по часовой стрелке. Куда она теперь смотрит?','вправо',['влево','вниз','вверх'],'Поворот по часовой стрелке: вверх → вправо',1);SP('На куб смотрят строго сверху. Какую фигуру видно в проекции?','квадрат',['круг','треугольник','шестиугольник'],'Верхняя грань куба — квадрат',2);SP('Тень от шара на плоскости при освещении спереди обычно выглядит как:','круг',['квадрат','треугольник','отрезок'],'У шара круглая проекция',2);SP('Тень от конуса сбоку обычно выглядит как:','треугольник',['круг','квадрат','прямоугольник'],'Сбоку виден треугольный силуэт',2);SP('Букву R отразили в вертикальном зеркале. Что произойдёт с изображением?','левая и правая стороны поменяются местами',['верх и низ поменяются местами','буква не изменится','буква повернётся на 90°'],'Вертикальное зеркало меняет левое и правое',3);SP('Закрытую коробку собрали из шести одинаковых квадратов. Какая пространственная фигура получилась?','куб',['пирамида','цилиндр','конус'],'Шесть равных квадратных граней образуют куб',3);SP('Если квадрат повернуть на 45°, какая фигура получится на рисунке?','ромб',['треугольник','круг','трапеция'],'Повернутый квадрат часто выглядит как ромб',3);SP('Лист сложили пополам дважды, затем прорезали одно маленькое отверстие через все слои. Сколько отверстий будет после разворачивания?','4',['1','2','8'],'Два сгиба дают четыре слоя',3);
// Memory (8)
...
function renderPsych(mode){
  ...
  if(mode==='style'){
    optsEl.innerHTML=q.o.map((o,i)=>{return'<div class="likert-opt" data-wave86u-on-click="psychSelStyle('+i+')"><div class="likert-dot"></div>'+o.t+'</div>'}).join('');
  } else {
    optsEl.innerHTML=['Да, точно','Скорее да','Скорее нет','Нет'].map((lbl,i)=>'<div class="likert-opt" data-wave86u-on-click="psychSel('+i+')"><div class="likert-dot"></div>'+lbl+'</div>').join('');
  }
}

...
(function(){
  ...
  function deadFeature(name){ return /^(pomodoro|anki|wordle)$/.test(String(name||'')); }
  ...
  function renderPomodoro(){ if(deadFeature('pomodoro')){removeIf('wave91e-pomodoro-card');removeIf('wave91e-pomodoro-mini');return;}
    var state = readPomodoro();
  ...
  function mountPomodoro(){
    if(deadFeature('pomodoro')){removeIf('wave91e-pomodoro-card');return;}
  ...
  function mountPomodoroMini(){
    if(deadFeature('pomodoro')){removeIf('wave91e-pomodoro-mini');return;}
  ...
  function exportAnkiImport(){ if(deadFeature('anki')){toast('Экспорт в Anki отключён в wave92j.');return;}
  ...
  function mountAnkiCard(){
    if(deadFeature('anki')){removeIf('wave91e-anki-card');return;}
  ...
  function scheduleTicker(){
    if(deadFeature('pomodoro')){ if(timers.pomodoro) clearInterval(timers.pomodoro); timers.pomodoro=null; return; }
...
(function(){
  ...
  function boot(){patch();mount();scheduleNotification();r.wave91hUxGamification={version:W,keys:{notifications:K('notifications'),quiet:K('quiet_hour'),dailyQuiz:K('daily_quiz_'+day())},level:level(),seasonal:seasonal(),fgos:fgos(),auditSnapshot:function(){return{wave:W,grade:g(),hasCard:!!document.getElementById('wave91h-main-card')||!!document.getElementById('s-main'),level:level(),seasonal:seasonal(),heatCells:heatRows().length,fgos:fgos().length,quietActive:quietActive(),supports:['G1-notification-api','G2-knowledge-heatmap','G3-parent-pdf-report','G4-fgos-progress','G6-quiet-hour','G7-qr-codes','G8-daily-mini-banner','H1-levels','H3-seasonal-events','H4-marathon-leaderboard']}},openHeatmap:openHeat,openParentReport:parentReport,openQr:openQr}}
...
(function(){if(typeof window!=='undefined'){window.__wave92jFeaturepackDisabled={version:'wave92j',pomodoro:true,anki:true,wordle:true,difficultyTools:true};}})();

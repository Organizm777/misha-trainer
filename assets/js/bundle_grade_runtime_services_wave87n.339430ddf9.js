...
(function(){
  ...
  function renderBackupModal(mode){
    ...
    if(!isImport){
      ...
      body.querySelector('#wave29-backup-qr-btn').addEventListener('click', function(){ wave93eRecord('qr'); showQrPackage(code, '📱 QR-экспорт backup', 'Покажи QR на одном устройстве и импортируй на другом через сканер.', 'backup'); });
      var wave93eTelegramBtn = body.querySelector('#wave93e-backup-telegram'); if(wave93eTelegramBtn) wave93eTelegramBtn.addEventListener('click', function(){ wave93eOpenTelegram(code); });
      var wave93eHealthBtn = body.querySelector('#wave93e-backup-health'); if(wave93eHealthBtn) wave93eHealthBtn.addEventListener('click', function(){ wave93eShowDataStatus(); });
    }
    return ui.overlay;
  }


  /* --- wave93e_data_transfer.js --- */
  var WAVE93E_STATE_KEY = 'trainer_wave93e_data_state';
  function wave93eLsGet(key){ try { return localStorage.getItem(key); } catch(_e) { return null; } }
  function wave93eLsSet(key, value){ try { localStorage.setItem(key, String(value)); return true; } catch(_e) { return false; } }
  function wave93eParseJson(text, fallback){ try { return text ? JSON.parse(text) : fallback; } catch(_e) { return fallback; } }
  function wave93eState(){
    var state = wave93eParseJson(wave93eLsGet(WAVE93E_STATE_KEY), {}) || {};
    return {
      lastQrAt: String(state.lastQrAt || ''),
      lastTelegramAt: String(state.lastTelegramAt || ''),
      lastHealthAt: String(state.lastHealthAt || ''),
      telegramCount: toNum(state.telegramCount),
      qrCount: toNum(state.qrCount),
      healthCount: toNum(state.healthCount)
    };
  }
  function wave93eSaveState(state){ wave93eLsSet(WAVE93E_STATE_KEY, JSON.stringify(state || {})); }
  function wave93eNowIso(){ try { return new Date().toISOString(); } catch(_e) { return String(Date.now()); } }
  function wave93eRecord(kind){
    var state = wave93eState();
    if(kind === 'qr'){ state.lastQrAt = wave93eNowIso(); state.qrCount += 1; }
    else if(kind === 'telegram'){ state.lastTelegramAt = wave93eNowIso(); state.telegramCount += 1; }
    else if(kind === 'health'){ state.lastHealthAt = wave93eNowIso(); state.healthCount += 1; }
    wave93eSaveState(state);
    try { if(window.trainerEvents && window.trainerEvents.track) window.trainerEvents.track('wave93e_data_' + kind, { grade: gradeKey() }); } catch(_e) {}
    return state;
  }
  function wave93eLooksJson(value){
    value = String(value == null ? '' : value).trim();
    return value.charAt(0) === '{' || value.charAt(0) === '[';
  }
  function wave93eAppKeys(){
    var keys = [];
    try {
      for(var i=0;i<localStorage.length;i++){
        var k = localStorage.key(i);
        if(/^(trainer_|wave\d+|exam|rush|daily)/i.test(String(k || ''))) keys.push(k);
      }
    } catch(_e) {}
    keys.sort();
    return keys;
  }
  function wave93eByteSize(text){ try { return new Blob([String(text || '')]).size; } catch(_e) { return String(text || '').length * 2; } }
  function wave93eFormatBytes(bytes){
    bytes = toNum(bytes);
    if(bytes >= 1048576) return Math.round(bytes / 10485.76) / 100 + ' МБ';
    if(bytes >= 1024) return Math.round(bytes / 10.24) / 100 + ' КБ';
    return bytes + ' Б';
  }
  function wave93eQuietIntegrity(){
    var bad = [];
    wave93eAppKeys().forEach(function(k){
      var v = wave93eLsGet(k);
      if(!wave93eLooksJson(v)) return;
      try { JSON.parse(v); } catch(_e) { bad.push(k); }
    });
    return bad;
  }
  function wave93eStorageSummary(){
    var keys = wave93eAppKeys(), bytes = 0;
    keys.forEach(function(k){ bytes += wave93eByteSize(k) + wave93eByteSize(wave93eLsGet(k)); });
    return { keys: keys, keyCount: keys.length, bytes: bytes, badKeys: wave93eQuietIntegrity(), state: wave93eState() };
  }
  function wave93eBackupCode(){
    try {
      if(hasFn(window.encodeTransferPayload) && hasFn(window.getBackupSnapshot)) return encodeTransferPayload(getBackupSnapshot());
    } catch(_e) {}
    return '';
  }
  function wave93eBackupHash(code){ return hashText(String(code || '')); }
  function wave93eShortDate(value){
    if(!value) return '—';
    try { return new Date(value).toLocaleString('ru-RU', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' }); } catch(_e) { return String(value); }
  }
  function wave93eTelegramText(code){
    var frames = buildQrFrames(code, 'backup');
    var hash = wave93eBackupHash(code);
    var head = 'Trainer3 backup · ' + gradeTitle() + ' · ' + formatDate(Date.now(), { day:'2-digit', month:'2-digit', year:'numeric' }) + '\n' +
      'Хеш: ' + hash + ' · QR-кадров: ' + frames.length + '\n';
    if(String(code || '').length <= 2600) return head + '\n' + String(code || '');
    return head + '\nПолный backup-код слишком длинный для ссылки Telegram и уже скопирован в буфер. Вставь его следующим сообщением или используй QR-перенос.';
  }
  function wave93eOpenExternal(url){
    try {
      var a = document.createElement('a');
      a.href = url;
      a.target = '_blank';
      ...
      document.body.appendChild(a);
      a.click();
      setTimeout(function(){ try { a.remove(); } catch(_e) {} }, 0);
      return true;
    } catch(_e) {}
    try { return !!window.open(url, '_blank', 'noopener,noreferrer'); } catch(_e2) { return false; }
  }
  function wave93eOpenTelegram(code){
    code = String(code || wave93eBackupCode() || '');
    if(!code){ alert('Backup API ещё не готов. Открой страницу класса заново.'); return false; }
    wave93eRecord('telegram');
    copyText(code, '✅ Backup-код скопирован. В Telegram вставь его в сообщение после короткой заметки.');
    var url = 'https://t.me/share/url?url=' + encodeURIComponent(stripUrl()) + '&text=' + encodeURIComponent(wave93eTelegramText(code));
    var ok = wave93eOpenExternal(url);
    if(!ok) alert('Telegram не открылся автоматически. Backup-код уже скопирован — вставь его вручную.');
    return ok;
  }
  function wave93eShowQrTransfer(){
    var code = wave93eBackupCode();
    if(!code){ alert('Backup API ещё не готов. Открой страницу класса заново.'); return null; }
    wave93eRecord('qr');
    return showQrPackage(code, '📱 QR-перенос данных', 'Покажи QR на старом устройстве и открой «Резервная копия → Сканировать QR» на новом. Серия кадров защищена хешем ' + wave93eBackupHash(code) + '.', 'backup');
  }
  function wave93eShowTelegramSync(){
    var code = wave93eBackupCode();
    var frames = buildQrFrames(code, 'backup');
    var state = wave93eState();
    var ui = overlayCard('✈️ Telegram sync', 'Клиентский перенос без сервера: backup-код копируется локально, Telegram используется только как канал доставки.');
    var body = document.createElement('div');
    body.className = 'wave29-body';
    body.innerHTML =
      '<div class="wave29-section"><h4>Пакет переноса</h4><textarea readonly class="wave29-code" id="wave93e-telegram-code">' + esc(code) + '</textarea><div class="wave29-note" style="margin-top:8px">Класс: ' + esc(gradeTitle()) + ' · размер: ' + wave93eFormatBytes(wave93eByteSize(code)) + ' · хеш: ' + esc(wave93eBackupHash(code)) + ' · QR-кадров: ' + frames.length + '</div><div class="wave29-actions wave29-no-print"><button type="button" class="wave29-btn accent" id="wave93e-telegram-open">✈️ Открыть Telegram</button><button type="button" class="wave29-btn light" id="wave93e-telegram-copy">📋 Скопировать</button><button type="button" class="wave29-btn light" id="wave93e-telegram-qr">📱 QR</button><button type="button" class="wave29-btn light" id="wave93e-telegram-file">⬇️ .json</button></div></div>' +
      '<div class="wave29-section"><h4>Статус</h4><div class="wave29-pillrow"><span class="wave29-pill">Telegram: ' + esc(wave93eShortDate(state.lastTelegramAt)) + '</span><span class="wave29-pill">QR: ' + esc(wave93eShortDate(state.lastQrAt)) + '</span><span class="wave29-pill">Автобэкап: ' + esc(wave93eShortDate(wave93eLsGet('trainer_auto_backup_last_wave92j'))) + '</span></div><div class="wave29-note" style="margin-top:8px">Для импорта на другом устройстве открой нужный класс → меню → «Резервная копия» → вставь код или сканируй QR.</div></div>';
    ui.card.appendChild(body);
    body.querySelector('#wave93e-telegram-open').addEventListener('click', function(){ wave93eOpenTelegram(code); });
    body.querySelector('#wave93e-telegram-copy').addEventListener('click', function(){ copyText(code, '✅ Backup-код скопирован'); });
    body.querySelector('#wave93e-telegram-qr').addEventListener('click', wave93eShowQrTransfer);
    body.querySelector('#wave93e-telegram-file').addEventListener('click', function(){ if(hasFn(window.downloadBackupFile)) downloadBackupFile(); else copyText(code, '✅ Backup-код скопирован'); });
    return ui.overlay;
  }
  function wave93eStatusHtml(summary, estimate, swText){
    summary = summary || wave93eStorageSummary();
    var quota = estimate && estimate.quota ? wave93eFormatBytes(estimate.quota) : '—';
    var usage = estimate && estimate.usage ? wave93eFormatBytes(estimate.usage) : wave93eFormatBytes(summary.bytes);
    var pctText = estimate && estimate.quota ? Math.round((estimate.usage || 0) / estimate.quota * 100) + '%' : '—';
    return '<div class="wave29-grid">' +
      '<div class="wave29-metric"><div class="n">' + summary.keyCount + '</div><div class="l">ключей</div></div>' +
      '<div class="wave29-metric"><div class="n">' + esc(wave93eFormatBytes(summary.bytes)) + '</div><div class="l">данные</div></div>' +
      '<div class="wave29-metric"><div class="n">' + summary.badKeys.length + '</div><div class="l">битых JSON</div></div>' +
      '<div class="wave29-metric"><div class="n">' + esc(pctText) + '</div><div class="l">квота</div></div>' +
      '</div><div class="wave29-section"><h4>Хранилище</h4><div class="wave29-pillrow"><span class="wave29-pill">usage: ' + esc(usage) + '</span><span class="wave29-pill">quota: ' + esc(quota) + '</span><span class="wave29-pill">auto-backup: ' + esc(wave93eShortDate(wave93eLsGet('trainer_auto_backup_last_wave92j'))) + '</span><span class="wave29-pill">SW: ' + esc(swText || 'проверяю…') + '</span></div>' + (summary.badKeys.length ? '<div class="wave29-note" style="margin-top:8px">Повреждённые ключи: ' + esc(summary.badKeys.slice(0,8).join(', ')) + (summary.badKeys.length > 8 ? '…' : '') + '</div>' : '<div class="wave29-note" style="margin-top:8px">JSON-записи приложения читаются корректно.</div>') + '</div>';
  }
  function wave93eShowDataStatus(){
    var summary = wave93eStorageSummary();
    wave93eRecord('health');
    var ui = overlayCard('🩺 Данные и хранилище', 'Проверка integrity, auto-backup, квоты браузера и Service Worker без внешней синхронизации.');
    var body = document.createElement('div');
    body.className = 'wave29-body';
    body.innerHTML = '<div id="wave93e-data-status-box">' + wave93eStatusHtml(summary, null, (navigator.serviceWorker && navigator.serviceWorker.controller ? 'активен' : 'нет controller')) + '</div><div class="wave29-actions wave29-no-print"><button type="button" class="wave29-btn accent" id="wave93e-health-backup">💾 Открыть backup</button><button type="button" class="wave29-btn light" id="wave93e-health-integrity">🔍 Проверить integrity</button><button type="button" class="wave29-btn light" id="wave93e-health-prune">🧹 Очистить старые логи</button><button type="button" class="wave29-btn light" id="wave93e-health-sw">🔄 Проверить SW</button></div>';
    ui.card.appendChild(body);
    var box = body.querySelector('#wave93e-data-status-box');
    if(navigator.storage && navigator.storage.estimate){
      navigator.storage.estimate().then(function(est){ box.innerHTML = wave93eStatusHtml(summary, est, (navigator.serviceWorker && navigator.serviceWorker.controller ? 'активен' : 'нет controller')); }).catch(function(){});
    }
    body.querySelector('#wave93e-health-backup').addEventListener('click', function(){ renderBackupModal(); });
    body.querySelector('#wave93e-health-integrity').addEventListener('click', function(){
      var bad = wave93eQuietIntegrity();
      if(window.__wave92jAppSafety && typeof window.__wave92jAppSafety.checkIntegrity === 'function') { try { window.__wave92jAppSafety.checkIntegrity(); } catch(_e) {} }
      alert(bad.length ? ('⚠️ Повреждённых JSON-записей: ' + bad.length) : '✅ Повреждённых JSON-записей не найдено');
    });
    body.querySelector('#wave93e-health-prune').addEventListener('click', function(){
      var removed = [];
      if(window.__wave92jAppSafety && typeof window.__wave92jAppSafety.pruneOldLocalData === 'function') { try { removed = window.__wave92jAppSafety.pruneOldLocalData() || []; } catch(_e) {} }
      alert(removed.length ? ('Очищено старых записей: ' + removed.length) : 'Старых безопасных логов для очистки не найдено');
    });
    body.querySelector('#wave93e-health-sw').addEventListener('click', function(){
      if(!navigator.serviceWorker || !navigator.serviceWorker.getRegistration){ alert('Service Worker недоступен в этом браузере.'); return; }
      navigator.serviceWorker.getRegistration().then(function(reg){
        if(!reg){ alert('Service Worker не зарегистрирован.'); return; }
        if(reg.update) reg.update();
        alert('Проверка обновления Service Worker запущена. Если новая версия есть, появится штатное уведомление.');
      }).catch(function(){ alert('Не удалось проверить Service Worker.'); });
    });
    return ui.overlay;
  }
  ...
  window.wave93eDataTools = { wave:'wave93e', openQr: wave93eShowQrTransfer, openTelegram: wave93eShowTelegramSync, openStatus: wave93eShowDataStatus, open:function(kind){ if(kind === 'qr') return wave93eShowQrTransfer(); if(kind === 'telegram') return wave93eShowTelegramSync(); return wave93eShowDataStatus(); }, summary: wave93eStorageSummary, telegramShare: wave93eOpenTelegram };
  ...
  window.wave29Debug = {
    ...
    showQrTransfer: wave93eShowQrTransfer,
    showTelegramSync: wave93eShowTelegramSync,
    showDataStatus: wave93eShowDataStatus,
    dataHealthSummary: wave93eStorageSummary,
    canScanQr: canScanQr,

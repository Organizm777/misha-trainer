(function(){
  ...
  function navigationContext(){ try{ if(window.TrainerNavigationLog && typeof window.TrainerNavigationLog.read === 'function'){ return window.TrainerNavigationLog.read().slice(-8); } }catch(_){ } return []; }
  ...
  tracker.record = record; tracker.getLogs = getLogs; tracker.getNavigationContext = navigationContext; tracker.getSummary = function(){ return summarize(); }; tracker.clear = clear; tracker.export = exportLogs; tracker.setConfig = writeConfig; tracker.renderDashboardCard = renderDashboardCard; tracker.download = function(){ return downloadText('trainer-runtime-errors.json', exportLogs(), 'application/json;charset=utf-8'); };
  tracker.wave91j = { lightweightErrorTracking:true, navigationContext:true };

...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();r.addEventListener('load',function(){setTimeout(boot,0)},{once:true});if(typeof setInterval==='function'){timer=setInterval(function(){patch();mount()},700);if(typeof setTimeout==='function')setTimeout(function(){clearInterval(timer)},9000)}
})();
...
(function(){
  ...
  function visual(){
    ...
    var t = lower(text);
    var s = subj(), sid = lower(s && (s.id + ' ' + s.nm));
    if (/геом|geometry|треуг|окруж|радиус|диаметр|параллел|угол|трапец|координат|парабол|график/.test(sid + ' ' + t)) return geometrySvg(text);
    if (/геогр|geography|карта|климат|материк|росси|река|море|зон|страна/.test(sid + ' ' + t)) return geographySvg(text);
    if (/хим|chem|молекул|веществ|формул|кислот|основан|соль|спирт|бензол|этанол|аммиак|органич/.test(sid + ' ' + t)) return chemistrySvg(text);
    if (/физ|physics|ток|напряж|сопротив|линз|луч|маятник|рычаг|сила|давлен|скорост|ускор|энерг/.test(sid + ' ' + t)) return physicsSvg(text);
    return null;
  }
...
(function(){
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();r.addEventListener('load',function(){setTimeout(boot,0)},{once:true});if(typeof setInterval==='function'){timer=setInterval(function(){patch();mount()},1200);if(typeof setTimeout==='function')setTimeout(function(){clearInterval(timer)},10000)}
})();
